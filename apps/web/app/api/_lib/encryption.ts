import {
  createCipheriv,
  createDecipheriv,
  randomBytes,
  scryptSync,
} from 'crypto';

// Define the cryptographic constants.
const ALGORITHM = 'aes-256-gcm';
const IV_LENGTH = 16;
const SALT_LENGTH = 64;
const TAG_LENGTH = 16;
const ENCODING = 'hex';

/**
 * Encrypts a plaintext secret using the master key from the environment.
 */
export function encryptSecret(secret: string, masterKey: string): string {
  const salt = randomBytes(SALT_LENGTH);
  const key = scryptSync(masterKey, salt, 32);
  const iv = randomBytes(IV_LENGTH);
  const cipher = createCipheriv(ALGORITHM, key, iv);
  const encrypted = Buffer.concat([
    cipher.update(secret, 'utf8'),
    cipher.final(),
  ]);
  const tag = cipher.getAuthTag();

  return [
    salt.toString(ENCODING),
    iv.toString(ENCODING),
    tag.toString(ENCODING),
    encrypted.toString(ENCODING),
  ].join(':');
}

/**
 * Decrypts a stored secret string using the master key. This is the final,
 * robust version that correctly handles all type checks.
 */
export function decryptSecret(
  encryptedSecret: string | null | undefined,
  masterKey: string
): string | null {
  try {
    // --- THE DEFINITIVE FIX STARTS HERE ---

    // 1. Guard against null or undefined input.
    if (!encryptedSecret) {
      return null;
    }

    const parts = encryptedSecret.split(':');

    // 2. Guard against a malformed structure.
    if (parts.length !== 4) {
      console.error('Decryption failed: Invalid encrypted format.');
      return null;
    }

    // 3. De-structure the parts.
    const [saltHex, ivHex, tagHex, encryptedHex] = parts;
    
    // 4. Guard against any of the parts being empty or undefined.
    //    This is the step that makes TypeScript 100% confident.
    if (!saltHex || !ivHex || !tagHex || !encryptedHex) {
        console.error('Decryption failed: One of the encrypted parts is missing.');
        return null;
    }

    // --- AFTER THIS POINT, TYPESCRIPT KNOWS ALL VARIABLES ARE 'string' ---

    const salt = Buffer.from(saltHex, ENCODING);
    const iv = Buffer.from(ivHex, ENCODING);
    const tag = Buffer.from(tagHex, ENCODING);
    
    const key = scryptSync(masterKey, salt, 32);
    
    const decipher = createDecipheriv(ALGORITHM, key, iv);
    decipher.setAuthTag(tag);

    const decrypted = decipher.update(encryptedHex, 'hex', 'utf8') + decipher.final('utf8');
    
    return decrypted;
  } catch (error) {
    console.error(
      'Decryption failed. This may be due to a wrong master key or tampered data.',
      error
    );
    return null;
  }
}